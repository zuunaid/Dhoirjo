...
const SITE = {
  ...
  repo: 'Dhoirjo',          // repo name (case-sensitive)
  branch: 'main',
  postsPerPage: 10
};
...
/* Normalize taxonomy strings (trim, remove zero-width chars, NFC) */
function cleanTax(s){
  return String(s || '')
    .normalize('NFC')
...
/* Convert digits only in visible text nodes (don’t touch URLs/attrs) */
...
// ------- Bullet-proof Scroll-to-Top bootstrap (drop-in) -------
(function bootstrapScrollTop(){
  // 1) Ensure the button exists (create if missing)
  ...
  // 2) Ensure the CSS "visible" state exists (inject if missing)
  const needRule = (() => {
    try {
      return !Array.from(document.styleSheets).some(ss => {
        try {
          return Array.from(ss.cssRules || []).some(r => r.selectorText === '#scrollTopBtn.show');
        } catch { return false; } // cross-origin stylesheet
      });
    } catch { return true; }
  })();
  ...
  if (needRule) {
    ...
    style.textContent = `
      #scrollTopBtn {
        ...
        display: none; z-index: 10000; opacity: 0; pointer-events: none;
        transition: opacity .3s ease, background .2s ease;
      }
      #scrollTopBtn.show { display: block !important; opacity: 1; pointer-events: auto; }
    `;
    document.head.appendChild(style);
  }

  // 3) Initialize ONCE (even if your other inits call it too)
  if (!btn.dataset.scrollTopInit) {
    btn.dataset.scrollTopInit = '1';

    const threshold = 40; // low to quickly prove it works
    const toggle = () => {
    ...
    window.addEventListener('scroll', toggle, { passive: true });
    window.addEventListener('hashchange', toggle, { passive: true });
    ...
    // run once after DOM is parsed, and again after load (BFCache, restored positions)
    const run = () => toggle();
    ...
    // 4) Safety timer: re-check after 1s in case content height changes late
